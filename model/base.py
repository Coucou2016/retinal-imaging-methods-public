...
def _pretrained_path(filename: str) -> str:
    base = os.environ.get(
        "RETI_PIONEER_PRETRAINED_DIR",
        os.path.join(os.path.dirname(os.path.dirname(__file__)), "pretrained"),
    )
...
def get_EyeQ():
    ...
    weight_path = os.environ.get(
        ...
        _pretrained_path("fundus/DenseNet121_v3_v1.tar"),
    )
    if not os.path.isfile(weight_path):
        raise FileNotFoundError(
            f"EyeQ weights not found at {weight_path}. "
            "Download from the EyeQ repository and set RETI_PIONEER_EYEQ_WEIGHTS."
        )
    eyeq_pretrain = torch.load(weight_path, map_location=torch.device("cpu"), weights_only=False)
...
def get_VimS():
    ...
    vim_path = os.environ.get(
        "RETI_PIONEER_VIM_WEIGHTS",
        _pretrained_path("vim_s_midclstok_ft_81p6acc.pth"),
    )
    if not os.path.isfile(vim_path):
        raise FileNotFoundError(
            f"Vision Mamba weights not found at {vim_path}. "
            "Install mamba-ssm and download Vim-S weights, or train in fast_mode on pre-extracted features."
        )
    model = vim_small_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2()
    sd = torch.load(vim_path, map_location="cpu", weights_only=False)
