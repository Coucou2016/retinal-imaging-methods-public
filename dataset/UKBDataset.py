...
except ImportError:  # pragma: no cover - minimal env without full torchvision
    Compose = object  # type: ignore[misc, assignment]
...
class UKBDatasetFast:
    def __init__(self,
        ...
        data = np.load(os.path.join(path, "UKB_mqd.npz"), allow_pickle=True)
        ...
        self.m = self.m[:, mi]
        self.meta_names: List[str] = list(meta)
        n_rows = self.m.shape[0]
        if "pid" in data.files:
            self.pid = np.asarray(data["pid"])
        else:
            self.pid = np.arange(n_rows)
        # mean = self.m.mean(0)
        # std = self.m.std(0)
        # if "gender" in meta:
        ...
        yn_names = None
        for i in [0, 5, 10]:
            data = np.load(os.path.join(path, f"UKB_y{i}.npz"), allow_pickle=True)
            self.__setattr__(f"y{i}", data["y"])
            if yn_names is None and "yn" in data.files:
                yn_names = [str(x) for x in np.asarray(data["yn"]).tolist()]
            del data
        ...
        self.disease_names = yn_names or [
            "t2dm", "thyroid", "osteoporosis", "gout", "hyperlipemia", "hypertension"
        ]
    ...
    def set_target(self, y: Literal[0, 5, 10], disease: List[int] | List[str], incident_exclude_prior: bool = True):
        if len(disease) > 0 and isinstance(disease[0], str):
            ...
            if missing:
                raise ValueError(f"Unknown disease {missing}; have {self.disease_names}")
            disease = [self.disease_names.index(n) for n in disease]
        self.yy = self.ys[[0, 5, 10].index(y)]
        n_col = int(self.yy.shape[-1]) if self.yy.ndim > 1 else 1
        disease = [int(d) for d in disease if int(d) < n_col]
        if not disease:
            disease = [0]
